        ...
        try {
            ...
            if (!response[0].equalsIgnoreCase("200")) {
                ...
                if (ekoElectricityQueryResponse.getCode().equalsIgnoreCase("000")) {
                    ...
                    if(ekoElectricityQueryResponse.getContent().getError() != null){

                        electricityQueryResponse = new ElectricityQueryResponse();
                        electricityQueryResponse.setResponseDesc(ekoElectricityQueryResponse.getContent().getError());
                        electricityQueryResponse.setResponseCode(Response.FAILED.code);
                    }else {
                        ...
                        String BusinessUnit = ekoElectricityQueryResponse.getContent().getCustomer_District();
                        String customerName = ekoElectricityQueryResponse.getContent().getCustomer_Name();
                        String minimum_amount = ekoElectricityQueryResponse.getContent().getMinimum_Amount();
                        String wrong_billars_code = ekoElectricityQueryResponse.getContent().getWrongBillersCode();
                        ...
                        electricityQueryResponse.setBusinessUnit(ekoElectricityQueryResponse.getContent().getCustomer_District());
                        ...
                        String redisParam = address + "~" + minimumPurchase + "~" + BusinessUnit + "~" + ekoElectricityQueryResponse.getContent().getTariff() + "~" + customerName;
                        lemonpayRedisService.setValue(electricityQueryRequest.getPayerId(), redisParam, 3600L);
                    }
                } else {

                    electricityQueryResponse = new ElectricityQueryResponse();
                    electricityQueryResponse.setResponseDesc(ekoElectricityQueryResponse.getResponse_description());
                    electricityQueryResponse.setResponseCode(Response.FAILED.code);
                }
    ...
    public ElectricityProcessResponse payment(ProcessRequestdto processRequestdto, ElectricityProcessRequest electricityProcessRequest, String actionType) {
        ...
        if (response[0].equalsIgnoreCase("200")) {
            ...
            if (processResponse.getCode().equalsIgnoreCase("000")) {
                ...
                electricityProcessResponse.setCustomerArrears(processResponse.getArrearsBalance());
                //electricityProcessResponse.setDeductions(processResponse);
                electricityProcessResponse.setDisco("EKOEDC");
