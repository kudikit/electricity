...
import com.lemonpay.lemonpayvas.electricity.ikedc.enums.Response;
import com.lemonpay.lemonpayvas.electricity.ikedc.requestdto.ProcessRequestdto;
import com.lemonpay.lemonpayvas.electricity.ikedc.requestdto.QueryRequest;
import com.lemonpay.lemonpayvas.electricity.ikedc.requestdto.RequeryRequest;
import com.lemonpay.lemonpayvas.electricity.ikedc.responsedto.IkejaElectricityQueryResponse;
import com.lemonpay.lemonpayvas.electricity.ikedc.responsedto.ProcessResponse;
import com.lemonpay.lemonpayvas.electricity.ikedc.util.GenerateRequestId;
import com.lemonpay.lemonpayvas.electricity.ikedc.util.HTTPUtils;
...
//                        mainTokenData.setVat(String.valueOf(processResponse.getVat()));
    ...
    public ElectricityQueryResponse verify(ElectricityQueryRequest electricityQueryRequest){
        ...
        log.info("========================== the request to ikeja vtpas"+request);
        ...
        try {
            ...
            if (!response[0].equalsIgnoreCase("200")) {
                ...
                IkejaElectricityQueryResponse ikejaElectricityQueryResponse = gson.fromJson(response[1], IkejaElectricityQueryResponse.class);

                if (ikejaElectricityQueryResponse.getCode().equalsIgnoreCase("000")) {

                    if(ikejaElectricityQueryResponse.getContent().getError() != null){

                        electricityQueryResponse = new ElectricityQueryResponse();
                        electricityQueryResponse.setResponseDesc(ikejaElectricityQueryResponse.getContent().getError());
                        electricityQueryResponse.setResponseCode(Response.FAILED.code);
                    }else {

                        String address = ikejaElectricityQueryResponse.getContent().getAddress();
                        String minimumPurchase = ikejaElectricityQueryResponse.getContent().getMin_Purchase_Amount();
                        String BusinessUnit = ikejaElectricityQueryResponse.getContent().getDistrict();
                        String customerName = ikejaElectricityQueryResponse.getContent().getCustomer_Name();
                        String minimum_amount = ikejaElectricityQueryResponse.getContent().getMinimum_Amount();
                        String wrong_billars_code = ikejaElectricityQueryResponse.getContent().getWrongBillersCode();
                        ...
                        electricityQueryResponse.setBusinessUnit(ikejaElectricityQueryResponse.getContent().getDistrict());
                        electricityQueryResponse.setCustomerAddress(ikejaElectricityQueryResponse.getContent().getAddress());
                        electricityQueryResponse.setCustomerName(customerName);
                        electricityQueryResponse.setMinimumPurchase(ikejaElectricityQueryResponse.getContent().getMin_Purchase_Amount());
                        electricityQueryResponse.setTariff(ikejaElectricityQueryResponse.getContent().getTariff());

                        String redisParam = address + "~" + minimumPurchase + "~" + BusinessUnit + "~" + ikejaElectricityQueryResponse.getContent().getTariff() + "~" + customerName;
                        lemonpayRedisService.setValue(electricityQueryRequest.getPayerId(), redisParam, 3600L);
                    }
                } else {

                    electricityQueryResponse = new ElectricityQueryResponse();
                    electricityQueryResponse.setResponseDesc(ikejaElectricityQueryResponse.getResponse_description());
                    electricityQueryResponse.setResponseCode(Response.FAILED.code);
                }
    ...
    public ElectricityProcessResponse payment(ProcessRequestdto processRequestdto, ElectricityProcessRequest electricityProcessRequest, String actionType) {
        ...
        log.info("================== the payload sent to ikeja for payment is: "+gson.toJson(processRequestdto));
        ...
        if (response[0].equalsIgnoreCase("200")) {
            ...
            if (processResponse.getCode().equalsIgnoreCase("000")) {
                ...
                electricityProcessResponse.setCostOfUnit(String.valueOf(processResponse.getCostOfUnit()));
                electricityProcessResponse.setPenalty(String.valueOf(processResponse.getPenalty()));
                electricityProcessResponse.setAdminCharge(String.valueOf(processResponse.getAdministrativeCharge()));
                electricityProcessResponse.setMeterCost(String.valueOf(processResponse.getMeterCoset()));
                electricityProcessResponse.setReconnectionFee(String.valueOf(processResponse.getInstallationFee()));
                electricityProcessResponse.setLossOfFee(String.valueOf(processResponse.getLossOfRevenue()));
                electricityProcessResponse.setMeterServiceCharge(String.valueOf(processResponse.getMeterServiceCharge()));
                ...
                electricityProcessResponse.setCustomerArrears(processResponse.getArrearsBalance());
                //electricityProcessResponse.setDeductions(processResponse);
                electricityProcessResponse.setDisco("IKEDC");
