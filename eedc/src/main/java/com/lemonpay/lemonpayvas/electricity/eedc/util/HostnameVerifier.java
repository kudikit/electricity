package com.lemonpay.lemonpayvas.electricity.eedc.util;
...
public interface HostnameVerifier extends javax.net.ssl.HostnameVerifier {
    ...
    /**
     * Checks to see if the supplied hostname matches any of the supplied CNs or
     * "DNS" Subject-Alts. Most implementations only look at the first CN, and
     * ignore any additional CNs. Most implementations do look at all of the
     * "DNS" Subject-Alts. The CNs or Subject-Alts may contain wildcards
    ...
    /**
     * The DEFAULT_AND_LOCALHOST HostnameVerifier works like the DEFAULT one
     * with one additional relaxation: a host of "localhost",
     ...
     * Java 1.4, Sun Java 5, Sun Java 6. It's also pretty close to IE6. This
     * implementation appears to be compliant with RFC 2818 for dealing with
     * wildcards.
     * <p/>
     * The hostname must match either the first CN, or any of the subject-alts.
     * A wildcard can occur in the CN, and in any of the subject-alts. The one
     * divergence from IE6 is how we only check the first CN. IE6 allows a match
     * against any of the CNs present. We decided to follow in Sun Java 1.4's
     * footsteps and only check the first CN.
     * <p/>
     * A wildcard such as "*.foo.com" matches only subdomains in the same level,
     * for example "a.foo.com". It does not match deeper subdomains such as
     * "a.b.foo.com".
     */
     ...
     * server's certificate, not just the first one. This behaviour is identical
     * to IE6's behaviour.
     */
    ...
    abstract class AbstractVerifier implements HostnameVerifier {
         ...
         * CA's would never foolishly allow such a certificate to happen? Looks
         * like we're the only implementation guarding against this. Firefox,
         * Curl, Sun Java 1.4, 5, 6 don't bother with this check.
         */
